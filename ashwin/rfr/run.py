...
def run():
    ...
    if args.test:
        ...
        train_data = np.load(args.data_root)[:200]
        mask_data = np.load(args.mask_root)[:200]
        ...
        masks_tensor = torch.tensor(np.reshape(converted_masks, (-1,3,size,size)), dtype = torch.float)
        images_tensor = torch.tensor(np.reshape(train_data, (-1,3,size,size)), dtype = torch.float)
